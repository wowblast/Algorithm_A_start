...
class Node():
    def __init__(self, pos, children, parent, cost_total, heuristic):
        ...
        self.pos = pos
        self.children = [child[1] for child in children if (
            child[1] != parent and child[1][1] <= pos[1])]
        self.parent = parent
...
def getNeighbors(current_point, goal_point, G):
    ...
    for child in current_point.children:
        lista_possible_ways.append(Node(child, list(G.edges(
            child)), current_point.pos, 1+current_point.cost_total, (heuristic_funtion(child, goal_point))))
...
def algorithm_a_star(startpoint, goal, G):
    ...
    openlist = []
    closetlist = []
    current = Node(startpoint, list(G.edges(startpoint)),
                   (-8, -8), 0, heuristic_funtion(startpoint, goal))
